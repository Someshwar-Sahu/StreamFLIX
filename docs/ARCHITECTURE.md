...
## 11. Free Cloud Hosting & Keep-Alive Architecture (Render + Supabase)

### 11.1 Server & Database Dual Keep-Alive Ping
- **Render Inactivity Sleep**: Render's free Web Services spin down after 15 minutes of inactivity.
- **Supabase Database Pausing**: Supabase free PostgreSQL projects pause after 7 days of inactivity.
- **Combined Solution**: FastAPI `/health` endpoint executes an async `SELECT 1` database query. External 14-minute cron jobs (Cron-Job.org / UptimeRobot) ping `GET /health` to keep both the web service and the PostgreSQL database active 24/7 without manual intervention or local background processes.

### 11.2 Dynamic Client Base URL Architecture
- **Web App**: Checks `VITE_API_BASE_URL` environment variable at build/runtime, falling back to window origin or custom backend domain.
- **Mobile App**: `AsyncStorage` dynamically overrides `baseURL` via Axios request interceptors if a custom host URL is saved in Settings, defaulting to the production HTTPS Render endpoint (`https://<app-name>.onrender.com`).
- **Desktop App**: Electron packages the production Web build cleanly with `electron-builder` (`StreamFlix Setup.exe`).